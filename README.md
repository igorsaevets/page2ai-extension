...
<img src="assets/store/screenshot-2-openwebui-tools-1280x800.png" width="720" alt="Page2AI popup on Open WebUI docs — 80 KB extracted via official-md path">
...
Three fresh captures generated by running the built extension against the live sites (see `scripts/gen-screenshots-real.mjs`) — the popup values (profile, chars, tabs, ratio) are what the extension actually returned on each page today, not mockup numbers.

**AI vendor docs landing** — xAI Grok models overview at docs.x.ai. Vendor docs mix pricing tables, brand strip, and per-model callouts. Page2AI auto-detects `docs` profile and follows the site's `llms.txt` when present, so the popup shows `official-md` status — the shortest path to accurate content.

<img src="assets/store/screenshot-3-xai-grok-45-1280x800.png" width="720" alt="Page2AI popup on docs.x.ai/docs/models showing docs profile with official-md ratio 1.06">

**Docs page with an official `llms.txt`** — Open WebUI Tools reference. When a site publishes its own Markdown mirror, Page2AI discovers the sibling `.md` via `llms.txt`, checks the length ratio, and takes the short path when it wins — the popup here shows 80 KB extracted at `official-md ratio 1.32`, no tab walking needed.

<img src="assets/store/screenshot-2-openwebui-tools-1280x800.png" width="720" alt="Page2AI popup on docs.openwebui.com/features/extensibility/plugin/tools/ showing official-md path used">
