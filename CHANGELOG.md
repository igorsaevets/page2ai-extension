...
- **Extraction died — and could navigate the user away — on pages whose tab widgets change the URL without pushing a history entry** ([#10](https://github.com/igorsaevets/page2ai-extension/issues/10)), Google's developer docs (`ai.google.dev`) being the reproduced case. Tab capture undid URL drift with `history.back()`; on replace-style drift there is no entry to go back to, so the browser left the page entirely, the injected extractor died mid-run and the popup waited forever. Drift is now undone with `history.replaceState` — same document, a navigation is impossible by construction — which also covers the push-style drift `history.back()` was written for. Covered by a new e2e suite (`.e2e/e2e-url-drift.mjs`, in CI): replace-drift and push-drift fixtures, both must complete, capture both tab panels and leave the tab on the page. Also adds `.e2e/extract-url.mjs`, a dev utility that runs any live URL through the real extension pipeline and saves markdown + quality meta + the rendered page's innerText (this is the harness that caught the bug).
